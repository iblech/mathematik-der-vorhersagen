...
# auf mehr Glück hoffen.
#
# Anders als xor-4.py verwendet dieses Programm nicht die gewöhnliche L2-,
# sondern die Cross-Entropy-Kostenfunktion.
#
# Dieses Programm plottet nach dem Training die Feedforward-Funktion nach jedem
# Schritt des Gradientenabstiegs in einem Unterordner "images".
# Die einzelnen Plots kann man unter Linux mit dem Befehl
#
#     ffmpeg -i images/xor-%4d.png xor.mp4
#
...
eta = 0.5
print("Achtung. Dieses Programm funktioniert nicht out of the box, du musst mit eta herumspielen.")
os._exit(1)
...
# Liste aller V,W,b,c, die während des Gradientenabstiegs nacheinander entstehen.
states = []

for i in range(10000):
    # Aktuellen Stand ausgeben.
    print([feedforward(V,W,b,c, t[0])[3] for t in trainingData])
    states.append((V,W,b,c))
...
# Alle Zwischenstände plotten.
for i in range(len(states)):
    V,W,b,c = states[i]
    plt.clf()
    plot3d(fig, np.arange(0,1,0.05), np.arange(0,1,0.05), lambda x,y: feedforward(V,W,b,c, np.array([[x],[y]]))[3][0])
    plt.savefig("images/xor-%04d.png" % i)
